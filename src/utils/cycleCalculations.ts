import { addDays, differenceInDays, parseISO, format, isAfter, isBefore, isEqual } from 'date-fns';
...
export function getDaysUntilNextPeriod(predictedDate: Date | null, today: Date = new Date()): number | null {
  ...
  const days = differenceInDays(predictedDate, today);
...
export function getPhaseForDate(
  ...
  const applicableCycle = sorted.find(c => {
  ...
  if (day > averageCycleLength + 5) return null; // too far from a known period
