...
exports.CartesianAxis = exports.CartesianAxisDefaultProps = void 0;
...
Object.defineProperty(exports, "CartesianAxisDefaultProps", { enumerable: true, get: function () { return axisDefaults_1.CartesianAxisDefaultProps; } });
/**
...
const CartesianAxis = ({ tickCount = tickHelpers_1.DEFAULT_TICK_COUNT, xTicksNormalized, yTicksNormalized, labelPosition = "outset", labelOffset = { x: 2, y: 4 }, axisSide = { x: "bottom", y: "left" }, lineColor = "hsla(0, 0%, 0%, 0.25)", lineWidth = react_native_1.StyleSheet.hairlineWidth, labelColor = "#000000", formatYLabel = (label) => String(label), formatXLabel = (label) => String(label), yScale, xScale, font, isNumericalData = false, ix = [], }) => {
    ...
    const yAxisNodes = yTicksNormalized.map((tick) => {
        ...
        const labelLayout = (0, textLayout_1.getTextLayout)(contentY, font);
        ...
        {font
                ? canFitLabelContent && (<>
                {labelLayout.lines.map((line, index) => (<react_native_skia_1.Text key={`y-tick-${tick}-label-line-${index}`} color={typeof labelColor === "string" ? labelColor : labelColor.y} text={line} font={font} y={labelY + index * labelLayout.lineHeight} x={labelX}/>))}
              </>)
                : null}
    ...
    const xAxisNodes = xTicksNormalized.map((tick) => {
        ...
        const labelLayout = (0, textLayout_1.getTextLayout)(contentX, font);
        const labelWidth = labelLayout.width;
        ...
        {font && labelWidth && canFitLabelContent ? (<>
            {labelLayout.lines.map((line, index) => (<react_native_skia_1.Text key={`x-tick-${tick}-label-line-${index}`} color={typeof labelColor === "string" ? labelColor : labelColor.x} text={line} font={font} y={labelY + index * labelLayout.lineHeight} x={labelX}/>))}
          </>) : null}
      </react_1.default.Fragment>);
    });
